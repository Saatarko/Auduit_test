...
class Mainwindow(QMainWindow):
    ...
    def open_test(self, fio: str):
        ...
        msg.setText("Для прохождения теста Вам необходимо ответить на 37 вопросов. "
                    "Отвечать можно в любом порядке. Для этого Вам нужно нажать на соответствующий "
                    "вопрос. Он отобразится в окне справа. Под вопросом будут варианты ответа. "
                    "Вам нужно выбрать правильный вариант (правильных ответов может быть несколько). "
                    "Если вы уверены в своем выборе, то нажимайте на кнопку сохранить ответ. Отвеченный "
                    "вопрос выделяется зеленым цветом, но, даже в этом случае, если Вы засомневаетесь в ответе "
                    "и захотите его поменять, то Вы можете внести изменения в ответ и снова нажать на кнопку сохранить. "
                    "Если текущий вопрос Вам кажется сложным, то Вы можете перейти к любому другому вопросу,а к этому вернуться позднее. "
    ...
    def open_list_applicant(self):
        ...
        if not applicants:
            ...
            item = QListWidgetItem("Соискателей пока не было")
